...
#define MTK_SPI_LCM_PATCH

#ifdef MTK_SPI_LCM_PATCH
#define MTK_OVL_DECOUPLE_SUPPORT
static int mt_spi_lcm_line = 1;
static wait_queue_head_t spi_esd_ext_te_wq;
static atomic_t spi_esd_ext_te_event = ATOMIC_INIT(0);
static struct task_struct *spi_display_esd_check_task;
static int spi_display_te_worker_kthread(void *data);
#endif
...
#ifdef MTK_SPI_LCM_PATCH
    decouple_wdma_config.outputFormat       = eRGB565;
    decouple_wdma_config.dstPitch           = width * DP_COLOR_BITS_PER_PIXEL(eRGB565)/8;
#endif
	return 0;
}
...
#ifdef MTK_SPI_LCM_PATCH
    decouple_wdma_config.outputFormat       = eRGB565;
	decouple_wdma_config.dstPitch = primary_display_get_width() * DP_COLOR_BITS_PER_PIXEL(eRGB565) / 8;
#endif
...
unsigned int _need_wait_esd_eof(void)
{
	...
	if (_need_do_esd_check() == 0)
		...
		ret = 0;
#ifdef MTK_SPI_LCM_PATCH
	if(pgc->plcm->params->dsi.customization_esd_check_enable == 2)
	{
	        ret = 1;
	}
#endif
...
#else
	//ret = 0;
	if(pgc->plcm->params->dsi.customization_esd_check_enable == 1)
	{
		ret = disp_lcm_esd_check(pgc->plcm);
	}
	else if(pgc->plcm->params->dsi.customization_esd_check_enable == 2)
	{
		ret = 0;
	}
	return ret;
...
int primary_display_esd_recovery(void)
{
	...
	if(pgc->plcm->params->dsi.customization_esd_check_enable == 1)
	{
		ret = disp_lcm_esd_recover(pgc->plcm);
	}	
	else if(pgc->plcm->params->dsi.customization_esd_check_enable == 2)
	{
		ret = 0;
...
#ifdef MTK_SPI_LCM_PATCH
	mt_spi_lcm_line = lcm_param->dsi.LANE_NUM;
#endif
...
#ifdef MTK_SPI_LCM_PATCH
	spi_display_esd_check_task = kthread_create(spi_display_te_worker_kthread, NULL, "spi_te_check");
	init_waitqueue_head(&spi_esd_ext_te_wq);
#endif
...
#ifdef MTK_SPI_LCM_PATCH
            {
			extern void _display_custom_screen_update_wakeup(u8 *screen, u16 w, u16 h);
			_display_custom_screen_update_wakeup((u8 *)dc_vAddr[pgc->dc_buf_id] , primary_display_get_width(), primary_display_get_height());
            }
#endif			
...
#ifdef MTK_SPI_LCM_PATCH
extern void mt_spi_read_reg(u8 cmd, u8 *reg, u16 len);
...
static int spi_display_te_worker_kthread(void *data)
{
	...
	while (1) {
		if (wait_event_interruptible_timeout (spi_esd_ext_te_wq, atomic_read(&spi_esd_ext_te_event), HZ / 2) > 0) {
			ret = 0; /* esd check pass */
			printk("esd check pass\n");
		} else {
			ret = 1; /* esd check fail */		
			printk("esd check fail\n");
...
static irqreturn_t _spi_esd_check_ext_te_irq_handler(int irq, void *data)
{
	atomic_set(&spi_esd_ext_te_event, 1);
	wake_up_interruptible(&spi_esd_ext_te_wq);
	return IRQ_HANDLED;
}

int _disp_custom_lcm_switch_esd_mode(int mode)
{
	int ret = 0;
	int te_gpio;
	static int init_te_mode=0;
	...
	te_gpio = pgc->plcm->params->dsi.lcm_ext_te_monitor;
	printk("switch esd mode to %d, te_gpio=%d\n", mode, te_gpio);
	te_gpio |= 0x80000000;
	...
	if (mode == 1) {
		...
		mt_set_gpio_mode(te_gpio, 0);

		/* 2.register eint */
		node = of_find_compatible_node(NULL, NULL, "mediatek, SPI_TE-eint");
		if (node) {
			/* DISPMSG("node 0x%x\n", node); */
			of_property_read_u32_array(node, "debounce", ints, ARRAY_SIZE(ints));
			/* mt_gpio_set_debounce(ints[0], ints[1]); */
			...
			irq = irq_of_parse_and_map(node, 0);
			if (request_irq(irq, _spi_esd_check_ext_te_irq_handler, IRQF_TRIGGER_RISING, "SPI_TE-eint", NULL)){
				printk("[ESD]SPI EINT IRQ LINE NOT AVAILABLE!!\n");
			}
		} else {
			DISPERR("[ESD][%s] SPI can't find SPI_TE eint compatible node\n", __func__);
		}
	} 
	else if (mode == 0) {
		struct device_node *node = NULL;
		...
		if (node) {
			...
			DISPERR("[ESD][%s] can't find SPI_TE eint compatible node\n",__func__);
		}
		mt_set_gpio_mode(te_gpio, 0);
	}
	init_te_mode = 1;
...
void _disp_custom_lcm_vsync(void){
	dpmgr_signal_event(pgc->dpmgr_handle, DISP_PATH_EVENT_IF_VSYNC);
}

unsigned int _disp_custom_lcm_get_line(void)
{
	switch(mt_spi_lcm_line){
		case LCM_FOUR_LANE:
			return 4;
		case LCM_THREE_LANE:
			return 3;
...
unsigned int _disp_custom_lcm_get_clock_h(void)
{
	unsigned int pclk = 1;

	if (pgc->plcm->params){	
		pclk = pgc->plcm->params->dsi.pll_div1;
	}

	return pclk;
}

unsigned int _disp_custom_lcm_get_clock_l(void)
{
	unsigned int pclk = 1;

	if (pgc->plcm->params){	
		pclk = pgc->plcm->params->dsi.pll_div2;
	}

	return pclk;
}

void _disp_custom_lcm_check_esd(void){
	if (pgc->plcm->params){	
		if(pgc->plcm->params->dsi.customization_esd_check_enable == 1){
			unsigned char cmd;
			unsigned char ack[8];
			
			cmd = pgc->plcm->params->dsi.lcm_esd_check_table[0].cmd;
			if( cmd != 0 ){
				mt_spi_read_reg(cmd, ack, 2);
				printk("lcm_check_esd: %x %x\n",ack[0], pgc->plcm->params->dsi.lcm_esd_check_table[0].para_list[0]);
				if(ack[0] != pgc->plcm->params->dsi.lcm_esd_check_table[0].para_list[0]){
					disp_lcm_esd_recover(pgc->plcm);
				}
			}
		}
		else if(pgc->plcm->params->dsi.customization_esd_check_enable == 2){
			_disp_custom_lcm_switch_esd_mode(1);
		}
	}
}
#endif
